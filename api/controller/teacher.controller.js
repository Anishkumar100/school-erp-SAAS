...
module.exports = {
    ...
    loginTeacher: async (req, res) => {
        ...
        res.status(200).json({ success: true, message: "Success Login", user: { id: teacher._id, name: teacher.name, image_url: teacher.teacher_image, role: 'TEACHER' } });
    },

    getTeacherOwnDetails: async(req, res) => {
        const teacher = await Teacher.findOne({_id: req.user.id, school: req.user.schoolId});
        if (!teacher) return res.status(500).json({ success: false, message: "Teacher data not available" });
    ...
    isTeacherLoggedIn: async(req, res) => {
        try {
            ...
            res.status(500).json({ success: false, message: "Server Error in Teacher Logged in check." });
        }
    }
