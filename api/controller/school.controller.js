...
module.exports = {
    // This is the protected route for admins/logged-in schools
    ...
    // ✅ NEW: Public-safe function for the gallery
    getSchoolGallery: async (req, res) => {
        try {
            // This only selects the public-safe fields
            const schools = await School.find().select([
                'school_name',
                'school_image'
            ]);
            res.status(200).json({ success: true, message: "Fetched school gallery successfully", data: schools });
        } catch (error) {
            console.error("Error in getSchoolGallery:", error);
            res.status(500).json({ success: false, message: "Server Error while fetching school gallery" });
        }
    },
    ...
    registerSchool: async (req, res) => {
        ...
        form.parse(req, async (err, fields, files) => {
            try {
                ...
                res.status(201).json({ success: true, message: "School registered successfully", data: savedSchool });
